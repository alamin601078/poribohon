const loadAllPost = async (cadId = `posts`) => {
 ...
 const suchPost = async (cadid) => {
    const resp = await fetch(`https://openapi.programming-hero.com/api/retro-forum/posts?category=${cadid}`)
    const surch = await resp.json();
    console.log(surch.posts)
    const suchCad = postdataall(surch.posts)
